...
while (@states) {
	...
	$seen{$key} = $heat;
	if (length($dir) && $steps < 4) {
		if ($dir eq 'U') {
			push @states, [ $x, $y - 1, "U", $steps + 1, $heat + $map[$y - 1][$x] ] if $y > 0;
		}
		elsif ($dir eq 'D') {
			push @states, [ $x, $y + 1, "D", $steps + 1, $heat + $map[$y + 1][$x] ] if $y < $#map;
		}
		elsif ($dir eq 'L') {
			push @states, [ $x - 1, $y, "L", $steps + 1, $heat + $map[$y][$x - 1] ] if $x > 0;
		}
		elsif ($dir eq 'R') {
			push @states, [ $x + 1, $y, "R", $steps + 1, $heat + $map[$y][$x + 1] ] if $x < ($width - 1);
		}
	}
	else {
		if ($y > 0 && $dir ne 'D') {
