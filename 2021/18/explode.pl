...
while (@numbers > 1) {
	...
	my $y = shift @numbers;
	my $r = [ $x, $y, undef ];
	Scalar::Util::weaken($x->[2] = $r);
	Scalar::Util::weaken($y->[2] = $r);
	reduce_num $r;
	printf STDERR "Addition result: %s\n", fmtnum($r);
	unshift @numbers, $r;
}

my $result = shift @numbers;

printf STDERR "Final result: %s\n", fmtnum($result);
...
sub reduce_num ($n) {
	{
		printf STDERR "Looking to reduce: %s\n", fmtnum($n);
		if (explode $n) {
			redo;
		}
		if (splitnum $n) {
...
sub explode ($n) {
	...
	say STDERR "Nothing to explode";
	return !1;
}

sub splitnum ($n) {
	say STDERR "Checking split ... ";
	while (defined $n) {
		if (ref $n->[0]) {
			$n = $n->[0];
			next;
