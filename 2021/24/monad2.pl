...
sub exec_operation {
	...
	eval (q{
		while (defined(my $state = each %state)) {
			...
			#}
			if (!defined $newstate{$newstate} || $newstate{$newstate} > $input) {
	...
	if (my ($var) = /^\s*inp\s+([wxyz])\s*$/) {
		...
		while (defined(my $state = each %state)) {
			...
			for my $add ( @add ) {
				...
				if (!defined $newstate{$newstate} || $newstate{$newstate} > $newinput) {
...
for my $state (keys %state) {
	...
	if ($state[3] == 0) {
		...
		#say STDERR "(valid)\n";
		if (!defined $smallest || $input < $smallest) {
			$smallest = $input;
		}
	}
	#else {
...
say "Biggest valid serial: $smallest";
