...
class State {
	...
	method move($dir) {
		...
		return State->new(pos => $new, turn => $turn + 1);
	}
	...
	method check_viable () {
		...
		my $key = sprintf "%s:%d", $pos->GetHashCode(), $turn % $blizlcm;
...
while (@states + @next_batch + @missed_guess) {
	if (@states < 1) {
		if (@next_batch < 1) {
			...
			$guess = List::Util::min map { $_->turn + $_->goal_dist } @missed_guess;
			my @mg = @missed_guess;
			undef @missed_guess;
			for my $mg (@mg) {
				if ($mg->turn + $mg->goal_dist <= $guess) {
					push @states, $mg;
				}
				else {
			...
			$guess = $states[0]->turn + $states[0]->goal_dist;
	...
	#printf STDERR "[%d] Processing state pos=%s turn=%d dist=%d (best=%s)\n", scalar(@states), $state->pos->GetHashCode(), $state->turn, $state->goal_dist, ($best_finish//"<none>");
	$state->remember();
	if ($state->is_goal) {
		if (!defined($best_finish) || $state->turn < $best_finish) {
			...
			my @keep = grep { $_->turn + $_->goal_dist < $best_finish; } @states;
			printf STDERR "\tNew best time %d, dropped %d states\n", $best_finish, scalar(@states) - scalar(@keep);
			@states = @keep;
		}
		next;
	}
	if (defined($best_finish) && ($state->turn + $state->goal_dist) >= $best_finish) {
	...
	for my $move (UP, LEFT, DOWN, RIGHT, WAIT) {
		...
		if ($ns->check_viable()) {
			#say STDERR "accepted";
			$ns->remember();
			if ($ns->turn + $ns->goal_dist > $guess) { push @missed_guess, $ns; }
