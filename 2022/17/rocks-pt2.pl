...
use constant ROCK_COUNT => 1000000000000;
...
while ($rocks < ROCK_COUNT) {
	# Be on the look out for a repeating pattern whenever the jetstream has looped. If we've entered a repeat, we can streamline a hell of a lot.
	if (length($grid) > 16) {
		my $seen_key = sprintf "%s:%s", $jetix, substr($grid, -5);
		if ($pos_seen{$seen_key}) {
			say STDERR "Repetition detected, fast-forwarding...";
			my $period = $rocks - $pos_seen{$seen_key}[0];
			my $dht = length($grid) - $pos_seen{$seen_key}[1];
			say STDERR "Repetition interval: $period, delta: $dht";
			my $remain = ROCK_COUNT - $rocks;
			my $cycles = int($remain / $period);
			$rocks = ROCK_COUNT - ($remain % $period);
			my $cycleheight = $dht * $cycles;
			$repeat_fwd += $cycleheight;
			%pos_seen = ();
		}
		else {
			$pos_seen{$seen_key} = [$rocks, length($grid)]; # Record the rock count and height this is seen at.
		}
	}
...
my $score = $repeat_fwd + length $grid;
